...
void log(const std::string &msg) { OutputDebugStringA((msg + "\n").c_str()); }
...
VulkanRenderSystem::VulkanRenderSystem(SDL_Window *window, uint32_t width, uint32_t height, uint32_t logW,
                                       uint32_t logH, Services &services)
    : System(services), window(window)
{
    this->width = width;
    this->height = height;
    this->logW = logW;
...
bool VulkanRenderSystem::initialize()
{
    ...
    vertexBuffer = createBuffer(VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT, 0,
...
void VulkanRenderSystem::shutdown()
{
    ...
    for (auto &res : frameResources)
    {
        ...
        vkFreeMemory(device, res.renderTarget.memory, nullptr);
...
void VulkanRenderSystem::beginFrame()
{
    ...
    VkBufferDeviceAddressInfo vertBdaInfo{.sType = VK_STRUCTURE_TYPE_BUFFER_DEVICE_ADDRESS_INFO,
    ...
    std::array<Barrier, 2> layoutBarriers{
        ...
        {.image = depthImage,
         .srcStageMask = VK_PIPELINE_STAGE_2_EARLY_FRAGMENT_TESTS_BIT |
                         VK_PIPELINE_STAGE_2_LATE_FRAGMENT_TESTS_BIT, // both specified to control memory access at both
                                                                      // stages (write)
    ...
    vkCmdBindDescriptorSets(res.commandBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, spritePipeline.layout, 0, sets.size(),
                            sets.data(), 0, nullptr);
}
...
void VulkanRenderSystem::endFrame()
{
    ...
    vkCmdPipelineBarrier(res.commandBuffer, VK_PIPELINE_STAGE_HOST_BIT,
                         VK_PIPELINE_STAGE_DRAW_INDIRECT_BIT | VK_PIPELINE_STAGE_VERTEX_SHADER_BIT |
                             VK_PIPELINE_STAGE_FRAGMENT_SHADER_BIT,
                         0, 1, &barrier, 0, nullptr, 0, nullptr);

    // setup the attachments (color and depth) and begin rendering (dynamic rendering)
    VkRenderingAttachmentInfo colorAttachInfo{.sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO,
    ...
    VkRenderingAttachmentInfo depthAttachInfo{.sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO,
    ...
    VkRenderingInfo renderingInfo{.sType = VK_STRUCTURE_TYPE_RENDERING_INFO,
                                  .renderArea{.offset{.x = 0, .y = 0}, .extent{.width = logW, .height = logH}},
                                  .layerCount = 1,
    ...
    VkRect2D scissor{.offset{.x = 0, .y = 0}, .extent{.width = logW, .height = logH}};
    ...
    vkCmdDrawIndexedIndirect(res.commandBuffer, res.indirectDraws.buffer, 0, res.numDraws,
    ...
    if constexpr (Config::IsStandaloneMode())
    {
        ...
        VkResult acquireResult = vkAcquireNextImageKHR(
            device, swapchain, UINT64_MAX, imageReadySemaphores[imageReadySemaphoreIndex], VK_NULL_HANDLE, &imageIndex);
        ...
        std::array<Barrier, 2> blitToSwapTransitions{
            Barrier{.image = res.renderTarget.handle,
                    .srcStageMask = VK_PIPELINE_STAGE_2_COLOR_ATTACHMENT_OUTPUT_BIT,
        ...
        const float scale = shouldIntegerScale ? std::floor(std::min(swapchainWidth / static_cast<float>(logW),
                                                                     swapchainHeight / static_cast<float>(logH)))
                                               : std::min(swapchainWidth / static_cast<float>(logW),
                                                          swapchainHeight / static_cast<float>(logH));
        ...
        vkCmdClearColorImage(res.commandBuffer, swapchainImages[imageIndex], VK_IMAGE_LAYOUT_TRANSFER_DST_OPTIMAL,
                             &clearColor, 1, &range);
        ...
        VkImageBlit2 blitRegion{
            ...
            .srcOffsets = {{.x = 0, .y = 0, .z = 0},
                           {.x = static_cast<int32_t>(logW), .y = static_cast<int32_t>(logH), .z = 1}},
            .dstSubresource{.aspectMask = VK_IMAGE_ASPECT_COLOR_BIT, .mipLevel = 0, .layerCount = 1},
            .dstOffsets = {{.x = xOffset, .y = yOffset, .z = 0},
                           {.x = xOffset + xScaled, .y = yOffset + yScaled, .z = 1}}};
        ...
        std::array<Barrier, 1> barriers{Barrier{.image = swapchainImages[imageIndex],
    ...
    // signals for work completion
    auto createSignals = [this]()
    {
        if constexpr (Config::IsStandaloneMode())
        {
            return std::array{
    ...
    constexpr VkPipelineStageFlags2 imageStageFlags = Config::ExecSelect(
        VK_PIPELINE_STAGE_2_BLIT_BIT | VK_PIPELINE_STAGE_2_CLEAR_BIT, VK_PIPELINE_STAGE_2_ALL_COMMANDS_BIT);
    VkSemaphoreSubmitInfo imageAcquireWaitInfo{.sType = VK_STRUCTURE_TYPE_SEMAPHORE_SUBMIT_INFO,
                                               .semaphore = imageReadySemaphores[imageReadySemaphoreIndex],
                                               .stageMask = imageStageFlags};

    auto semaphoreSignals = createSignals();

    // keyed mutex setup for D3D11 interop
    ...
    VkWin32KeyedMutexAcquireReleaseInfoKHR keyedMutexInfo{
        ...
        .acquireCount = 1,
        .pAcquireSyncs = &res.renderTarget.memory,
        .pAcquireKeys = &acquireKey,
        .pAcquireTimeouts = &timeout,
        .releaseCount = 1,
        .pReleaseSyncs = &res.renderTarget.memory,
        .pReleaseKeys = &releaseKey};

    VkSubmitInfo2 submitInfo{.sType = VK_STRUCTURE_TYPE_SUBMIT_INFO_2,
                             .pNext = Config::ExecSelect(nullptr, &keyedMutexInfo),
...
void VulkanRenderSystem::update(Node &node)
{
    ...
    if (camNodePos.x > logW + sc->getSize().x || camNodePos.x < -sc->getSize().x)
        return;
    ...
    res.instances[instanceIdx] = {.vertexBufferAddress = res.vertBufferAddr,
...
bool VulkanRenderSystem::initializeVulkan()
{
    ...
    if constexpr (Config::IsStandaloneMode())
    {
        ...
        if (!createWorkSemaphores())
        {
            showError("Unable to create work-complete semaphores (per swapchain image)");
            return false;
        }
    }
    else
    {
        if (!createWorkSemaphores())
        {
            showError("Unable to create work-complete semaphores (per frame-in-flight)");
            return false;
        }
    }
    ...
    {
        if (!createInternalTargets())
        {
            showError("Unable to create internal render targets");
            return false;
        }
    }
    else
    {
        if (!importD3DTextures())
        {
            showError("Unable to import D3D11 render targets");
            return false;
        }
    }
    if (!createDepthTarget())
    {
        showError("Unable to create depth image");
        return false;
    }
...
bool VulkanRenderSystem::createVulkanInstance()
{
    ...
    // if using SDL window, get required extensions
    if (Config::IsStandaloneMode())
    {
...
bool VulkanRenderSystem::findGraphicsQueue()
{
    ...
    {
        // ensure it has presentation support (only check if in standalone mode)
        VkBool32 hasPresentSupport = Config::ExecSelect(VK_FALSE, VK_TRUE);
        if constexpr (Config::IsStandaloneMode())
        {
...
bool VulkanRenderSystem::createDevice(VkPhysicalDevice physicalDevice)
{
    ...
    VkPhysicalDeviceVulkan14Features supportedFeatures14{.sType = VK_STRUCTURE_TYPE_PHYSICAL_DEVICE_VULKAN_1_4_FEATURES,
                                                         .pNext = nullptr};
    ...
    if (!supportedFeatures13.dynamicRendering || !supportedFeatures13.synchronization2 ||
        ...
        !supportedFeatures.features.samplerAnisotropy || !supportedFeatures.features.multiDrawIndirect ||
    ...
    VkDeviceQueueCreateInfo gfxQueueInfo{.sType = VK_STRUCTURE_TYPE_DEVICE_QUEUE_CREATE_INFO,
...
bool VulkanRenderSystem::initializeVMA()
{
    ...
    VmaAllocatorCreateInfo vmaAllocInfo{.flags = VMA_ALLOCATOR_CREATE_BUFFER_DEVICE_ADDRESS_BIT,
...
bool VulkanRenderSystem::createSwapchain(uint32_t width, uint32_t height)
{
    ...
    {
        VkImageViewCreateInfo imgViewInfo{.sType = VK_STRUCTURE_TYPE_IMAGE_VIEW_CREATE_INFO,
...
bool vks::VulkanRenderSystem::createWorkSemaphores()
{
    ...
    {
        VkExportSemaphoreCreateInfo exportSemaphoreInfo{.sType = VK_STRUCTURE_TYPE_EXPORT_SEMAPHORE_CREATE_INFO,
                                                        .handleTypes =
...
VkShaderModule VulkanRenderSystem::createShaderModule(const std::string &fileName, shaderc_shader_kind kind) const
{
    ...
    VkShaderModuleCreateInfo moduleCreateInfo{
        .sType = VK_STRUCTURE_TYPE_SHADER_MODULE_CREATE_INFO, .codeSize = shaderSize, .pCode = result.cbegin()};
...
Pipeline VulkanRenderSystem::createGraphicsPipeline(const ShaderSet &shaderSet, const PipelineConfig &config) const
{
    ...
    VkPipelineVertexInputStateCreateInfo vertInputInfo{.sType =
                                                           VK_STRUCTURE_TYPE_PIPELINE_VERTEX_INPUT_STATE_CREATE_INFO};

    // input assembly, we'll be drawing triangle lists
    std::array<VkPrimitiveTopology, 2> topologyMap{VK_PRIMITIVE_TOPOLOGY_TRIANGLE_LIST,
    ...
    // rasterizer settings
    VkPipelineRasterizationStateCreateInfo rasterInfo{.sType =
...
bool VulkanRenderSystem::createSyncResources()
{
    ...
    VkSemaphoreCreateInfo semaphoreInfo{.sType = VK_STRUCTURE_TYPE_SEMAPHORE_CREATE_INFO, .pNext = &semaphoreTypeInfo};
...
Buffer VulkanRenderSystem::createBuffer(VkBufferUsageFlags usage, VkBufferCreateFlags flags, size_t byteSize,
                                        void *initData = nullptr)
{
    VkBufferCreateInfo buffInfo{.sType = VK_STRUCTURE_TYPE_BUFFER_CREATE_INFO,
                                .flags = flags,
                                .size = byteSize,
                                .usage = usage,
    ...
    Buffer newBuff;
    if (vmaCreateBuffer(vmaAllocator, &buffInfo, &allocInfo, &newBuff.buffer, &newBuff.allocation, nullptr) !=
        VK_SUCCESS)
    {
...
{
    VkExternalMemoryImageCreateInfo externalImageInfo{.sType = VK_STRUCTURE_TYPE_EXTERNAL_MEMORY_IMAGE_CREATE_INFO,
    ...
    for (auto &res : frameResources)
    {
        ...
        VkMemoryDedicatedAllocateInfo dedicatedInfo{.sType = VK_STRUCTURE_TYPE_MEMORY_DEDICATED_ALLOCATE_INFO,
                                                    .pNext = nullptr,
                                                    .image = res.renderTarget.handle};

        VkExportMemoryAllocateInfo exportAllocInfo{.sType = VK_STRUCTURE_TYPE_EXPORT_MEMORY_ALLOCATE_INFO,
                                                   .pNext = &dedicatedInfo,
                                                   .handleTypes = VK_EXTERNAL_MEMORY_HANDLE_TYPE_OPAQUE_WIN32_BIT};

        uint32_t memoryTypeIndex = findMemoryType(memReqs.memoryTypeBits, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT);
        VkMemoryAllocateInfo allocInfo{.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
                                       .pNext = &exportAllocInfo,
                                       .allocationSize = memReqs.size,
                                       .memoryTypeIndex = memoryTypeIndex};

        vkAllocateMemory(device, &allocInfo, nullptr, &res.renderTarget.memory);
        vkBindImageMemory(device, res.renderTarget.handle, res.renderTarget.memory, 0);
        ...
        // if (vmaCreateImage(vmaAllocator, &imageInfo, &allocInfo, &res.renderTarget.handle,
        // &res.renderTarget.allocation, nullptr) != VK_SUCCESS)
        //{
...

        {
            ...
            VkImageFormatProperties2 formatProps{.sType = VK_STRUCTURE_TYPE_IMAGE_FORMAT_PROPERTIES_2,
                                                 .pNext = &externalProps};

            vkGetPhysicalDeviceImageFormatProperties2(physicalDevice, &formatInfo, &formatProps);

            log(std::format("External memory features: {}",
                            externalProps.externalMemoryProperties.externalMemoryFeatures));
            log(std::format("Compatible handle types: {}",
                            externalProps.externalMemoryProperties.compatibleHandleTypes));
            log(std::format("Export from imported: {}",
                            externalProps.externalMemoryProperties.exportFromImportedHandleTypes));
...
bool vks::VulkanRenderSystem::createDepthTarget()
{
    // create depth image
    VkImageCreateInfo depthCreateInfo{.sType = VK_STRUCTURE_TYPE_IMAGE_CREATE_INFO,
                                      .imageType = VK_IMAGE_TYPE_2D,
                                      .format = depthFormat,
                                      .extent{.width = logW, .height = logH, .depth = 1},
                                      .mipLevels = 1,
    ...
    if (vmaCreateImage(vmaAllocator, &depthCreateInfo, &depthAllocInfo, &depthImage, &depthImageAllocation, nullptr) !=
        VK_SUCCESS)
    {
...
bool vks::VulkanRenderSystem::importD3DTextures()
{
    auto &renderTargets = d3drs->getRenderTargets();
    for (int i = 0; i < renderTargets.size(); ++i)
    {
        auto &rt = renderTargets[i];
        auto &res = frameResources[i];
        res.shareHandle = rt.backBuffSharedHandle;
        ...
        if (vkCreateImage(device, &imageInfo, nullptr, &res.renderTarget.handle) != VK_SUCCESS)
        {
            return false;
        }
        VkMemoryRequirements memReqs{};
        ...
        VkImportMemoryWin32HandleInfoKHR importInfo{.sType = VK_STRUCTURE_TYPE_IMPORT_MEMORY_WIN32_HANDLE_INFO_KHR,
                                                    ...
                                                    .handle = rt.backBuffSharedHandle};
        ...
        VkMemoryAllocateInfo allocInfo{.sType = VK_STRUCTURE_TYPE_MEMORY_ALLOCATE_INFO,
                                       .pNext = &dedicatedInfo,
                                       .allocationSize = memReqs.size, // must be >= memReqs.size
                                       .memoryTypeIndex = memoryTypeIndex};
        if (vkAllocateMemory(device, &allocInfo, nullptr, &res.renderTarget.memory) != VK_SUCCESS)
        {
            return false;
        }
        if (vkBindImageMemory(device, res.renderTarget.handle, res.renderTarget.memory, 0) != VK_SUCCESS)
        {
            return false;
        }
...
bool vks::VulkanRenderSystem::createIndirectDrawBuffers()
{
    ...
    for (auto &res : frameResources)
    {
        ...
        if (vmaMapMemory(vmaAllocator, res.indirectDraws.allocation, reinterpret_cast<void **>(&res.drawCommands)) !=
            VK_SUCCESS)
        {
            return false;
        }
        if (vmaMapMemory(vmaAllocator, res.instanceData.allocation, reinterpret_cast<void **>(&res.instances)) !=
            VK_SUCCESS)
        {
            return false;
        }
...
bool vks::VulkanRenderSystem::updatePerFrameDescriptors()
{
    for (auto &res : frameResources)
    {
        VkDescriptorBufferInfo dsWrite{.buffer = res.instanceData.buffer, .offset = 0, .range = VK_WHOLE_SIZE};

        VkWriteDescriptorSet writes{.sType = VK_STRUCTURE_TYPE_WRITE_DESCRIPTOR_SET,
                                    .dstSet = res.descSet,
                                    .dstBinding = 0,
                                    .dstArrayElement = 0,
                                    .descriptorCount = 1,
...
uint32_t vks::VulkanRenderSystem::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties)
{
...
void VulkanRenderSystem::loadModel()
{
        ...
        for (const Primitive &primitive : mesh.primitives)
        {
                ...
                if (access.type == TINYGLTF_TYPE_VEC3)
                {
                    for (int i = 0; i < access.count; ++i)
                    {
                        size_t offset = bv.byteOffset + access.byteOffset +
                                        i * ((bv.byteStride > 0) ? bv.byteStride : sizeof(glm::vec3));
            ...
            if (const auto &itr = primitive.attributes.find("TEXCOORD_0"); itr != primitive.attributes.end())
            {
                ...
                if (access.type == TINYGLTF_TYPE_VEC2)
                {
                    ...
                    {
                        size_t offset = bv.byteOffset + access.byteOffset +
                                        i * ((bv.byteStride > 0) ? bv.byteStride : sizeof(glm::vec2));
    ...
    vertexBuffer =
        createBuffer(vmaAllocator, VK_BUFFER_USAGE_STORAGE_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
                     sizeof(Vertex) * vertices.size(), vertices.data());
    indexBuffer =
        createBuffer(vmaAllocator, VK_BUFFER_USAGE_INDEX_BUFFER_BIT | VK_BUFFER_USAGE_SHADER_DEVICE_ADDRESS_BIT,
...
void VulkanRenderSystem::transitionImages(VkCommandBuffer commandBuffer, const std::span<Barrier> &barriers)
{
    ...
    for (int i = 0; i < barriers.size(); ++i)
    {
        ...
        vkBarriers[i] = {.sType = VK_STRUCTURE_TYPE_IMAGE_MEMORY_BARRIER_2,
...
std::vector<uint64_t> vks::VulkanRenderSystem::getSharedTextureHandles(int editorPID) const
{
    ...
    for (int i = 0; i < MaxFramesInFlight; ++i)
    {
        ...
        BOOL success = DuplicateHandle(GetCurrentProcess(), frameResources[i].shareHandle, editorProcess,
...
intptr_t vks::VulkanRenderSystem::exportWorkCompleteSemaphore(uint32_t index) const
{
    ...
    VkSemaphoreGetWin32HandleInfoKHR semHandleInfo{.sType = VK_STRUCTURE_TYPE_SEMAPHORE_GET_WIN32_HANDLE_INFO_KHR,
    ...
    if (!vkGetSemaphoreWin32HandleKHR ||
        vkGetSemaphoreWin32HandleKHR(device, &semHandleInfo, &semaphoreHandle) != VK_SUCCESS)
    {
...
ResourceId VulkanRenderSystem::loadTexture(const std::string &filepath, bool flipY)
{
    ...
    if (commandBuffer)
    {
        ...
        std::array<Barrier, 1> barrierTransfer = {Barrier{
            .image = img.handle,
        ...
        VkCopyBufferToImageInfo2 copyBuffToImg{.sType = VK_STRUCTURE_TYPE_COPY_BUFFER_TO_IMAGE_INFO_2,
        ...
        std::array<Barrier, 1> barrierColor = {Barrier{
            .image = img.handle,
