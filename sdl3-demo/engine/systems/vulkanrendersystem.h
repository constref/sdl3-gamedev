...
private:
    ...
    uint64_t timelineValue = MaxFramesInFlight - 1; // subtract 1 to ensure wait-for-ID / frame resource index start at
                                                    // 0 during render, avoids if (frameId < MaxFramesInFlight) check
...
public:
    VulkanRenderSystem(SDL_Window *window, uint32_t width, uint32_t height, uint32_t logW, uint32_t logH,
                       Services &services);
    ~VulkanRenderSystem();
    bool initialize();
